...
Instance.prototype.setDataValue = function(key, value) {
  this.changed(key, !this._isSameValue(key, value));
    ...
    if (!options.raw && this._customSetters[key]) {
      ...
      if (this.$options && this.$options.include && this.$options.includeNames.indexOf(key) !== -1) {
        ...
        if (!options.raw) {
          ...
          if (this.Model._hasPrimaryKeys && this.dataValues[key] && this.Model._isPrimaryKey(key)) {
            return this;
          }
...
Instance.prototype._isSameValue = function (key, value) {
  ...
  if (originalValue !== null && value !== null) {
    if (this.Model._hasDateAttributes && this.Model._isDateAttribute(key) && !value._isSequelizeMethod) {
      if (this.Model._hasDateOnlyAttributes && this.Model._isDateOnlyAttribute(key)) {
        return moment(value).isSame(originalValue, 'day');
      } else {
        return moment(value).isSame(originalValue);
      }
    }

    if (this.Model._isJsonAttribute(key)) {
...
Instance.prototype.reload = function(options) {
  ...
  .then(function(reload) {
    ...
    var self = this;
    _.forIn(reload.dataValues, function (value, key) {
